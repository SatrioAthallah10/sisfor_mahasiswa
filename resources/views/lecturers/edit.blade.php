...
<div class="max-w-2xl mx-auto border border-outline-variant bg-surface shadow-2xl relative overflow-hidden flex flex-col rounded">
    ...
    <div class="px-8 py-6 border-b border-outline-variant flex justify-between items-start bg-surface-bright shrink-0">
        <div>
            <h2 class="font-headline-md text-headline-md text-on-surface">{{ __('Edit Lecturer Account') }}</h2>
            <p class="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-widest mt-2">{{ __('Modify credentials or password for this account') }}</p>
        </div>
    ...
    <div class="px-8 py-8 flex-1">
        <form action="{{ route('lecturers.update', $lecturer->id) }}" method="POST" class="flex flex-col gap-8">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-col">
                ...
                <input class="bg-transparent border-0 border-b border-outline-variant focus:border-on-surface focus:ring-0 px-0 py-2 font-body-lg text-body-lg text-on-surface placeholder:text-on-surface-variant/40 transition-colors" id="name" name="name" value="{{ old('name', $lecturer->name) }}" placeholder="e.g. Dr. Jane Doe, M.T." type="text" required/>
            </div>
            ...
            <div class="flex flex-col">
                ...
                <input class="bg-transparent border-0 border-b border-outline-variant focus:border-on-surface focus:ring-0 px-0 py-2 font-body-lg text-body-lg text-on-surface placeholder:text-on-surface-variant/40 transition-colors font-mono" id="email" name="email" value="{{ old('email', $lecturer->email) }}" placeholder="lecturer@example.com" type="email" required/>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="flex flex-col">
                    ...
                    <input class="bg-transparent border-0 border-b border-outline-variant focus:border-on-surface focus:ring-0 px-0 py-2 font-body-lg text-body-lg text-on-surface placeholder:text-on-surface-variant/40 transition-colors" id="password" name="password" placeholder="{{ __('Leave blank to keep current') }}" type="password"/>
                </div>
                <div class="flex flex-col">
