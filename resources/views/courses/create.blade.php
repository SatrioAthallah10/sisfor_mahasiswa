...
<div class="max-w-2xl mx-auto border border-outline-variant bg-surface shadow-2xl relative overflow-hidden flex flex-col rounded">
    ...
    <div class="px-8 py-6 border-b border-outline-variant flex justify-between items-start bg-surface-bright shrink-0">
        <div>
            <h2 class="font-headline-md text-headline-md text-on-surface">{{ __('Create Course') }}</h2>
            <p class="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-widest mt-2">{{ __('Define academic courses and assign faculty lecturers') }}</p>
        </div>
    ...
    <div class="px-8 py-8 flex-1">
        <form action="{{ route('courses.store') }}" method="POST" class="flex flex-col gap-8">
            @csrf
            ...
            <div class="flex flex-col">
                ...
                <input class="bg-transparent border-0 border-b border-outline-variant focus:border-on-surface focus:ring-0 px-0 py-2 font-body-lg text-body-lg text-on-surface placeholder:text-on-surface-variant/40 transition-colors font-mono uppercase" id="code" name="code" value="{{ old('code') }}" placeholder="e.g. IF-301" type="text" required/>
            </div>
            ...
            <div class="flex flex-col">
                ...
                <input class="bg-transparent border-0 border-b border-outline-variant focus:border-on-surface focus:ring-0 px-0 py-2 font-body-lg text-body-lg text-on-surface placeholder:text-on-surface-variant/40 transition-colors" id="name" name="name" value="{{ old('name') }}" placeholder="e.g. Pemrograman Web" type="text" required/>
            </div>
